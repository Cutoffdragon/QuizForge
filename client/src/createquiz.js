...
export default function CreateQuiz() {
    ...
    const onSubmitForm = async () => {
        try {
            ...
            setPhaseState(5);
            const response = await fetch(process.env.REACT_APP_SOURCE_SITE + "/quizinfo", {
                method: "POST",
    ...
    const getQuizId = async () => {
        try {
            ...
            const response = await fetch(process.env.REACT_APP_SOURCE_SITE+ `/quizid?quizname=${query}`)
            const results = await response.json();
            console.log(results);
            window.location = process.env.REACT_APP_CLIENT_SITE + `/${results['quizId']}`
        } catch (err) {
            console.error(err.message);
            window.location = '/error'
        }
